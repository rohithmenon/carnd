...
int main()
{
  ...
  h.onMessage([&ukf,&target_x,&target_y, &ts](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          ukf.ReportMeasurement(meas_package_L);
          ...
          ukf.ReportMeasurement(meas_package_R);
          long long max_ts = max(timestamp_R, timestamp_L);
          if (max_ts > ts) {
              VectorXd state = ukf.Predict(std::chrono::microseconds(1500000));
              auto x = state[0];
              auto y = state[1];
              target_x = x;
              target_y = y;
